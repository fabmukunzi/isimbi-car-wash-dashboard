...
import { defaultProfile, noanalytics } from '@/src/utils/images';
...
const Analtytics = () => {
  ...
  const res = useGetAnalyticsQuery();
  const { data, isLoading }=res
  const totalValue = data?.groupedByCategory?.reduce(
    (total: number, exp: any) => total + exp.value,
    0
  );
  useEffect(()=>{
    res.refetch()
  ...
  return (
    <div className={`${data&&!data.isAnalytics?'':'h-screen'}`}>
      {data&&!data.isAnalytics ? (
        <div className='flex flex-col gap-20 mt-16 items-center '>
          <Image src={noanalytics} alt="image" width={200} height={200} />
          <Text className='text-2xl'>No Analytics Yet</Text>
        </div>
      ) : (
        <>
